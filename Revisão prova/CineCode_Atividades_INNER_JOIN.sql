...
select c.nome_cliente, a.data_aluguel from clientes c 
inner join alugueis a on
...
select f.titulo, a.data_aluguel from filmes f
inner join alugueis a on
f.id_filme = a.id_filme;
...
select c.nome_cliente, f.titulo, a.data_aluguel from clientes c
inner join alugueis a on
c.id_cliente = a.id_cliente
...
--   - título do filme;
--   - nome do ator.
select f.titulo, a.nome_ator from filmes f
inner join atuacoes at on
f.id_filme = at.id_filme
inner join atores a on
a.id_ator = at.id_ator;
...
--   INNER JOIN
--   WHERE
select c.nome_cliente, f.titulo, f.genero, a.data_aluguel from clientes c
inner join alugueis a on
c.id_cliente = a.id_cliente
inner join filmes f on
f.id_filme = a.id_filme
...
select f.titulo, a.nome_ator, a.nacionalidade from filmes f
inner join atuacoes at on
f.id_filme = at.id_filme
inner join atores a on
a.id_ator = at.id_ator
...
select a.nome_ator, f.titulo, f.ano_lancamento from atores a
inner join atuacoes at on
a.id_ator = at.id_ator
inner join filmes f on
f.id_filme = at.id_filme
...
select c.nome_cliente, f.titulo, a.nota, a.data_aluguel from clientes c
inner join alugueis a on
c.id_cliente = a.id_cliente
inner join filmes f on
f.id_filme = a.id_filme
...
select c.nome_cliente, f.titulo, a.nota from clientes c
inner join alugueis a on
c.id_cliente = a.id_cliente
inner join filmes f on
f.id_filme = a.id_filme
...
select c.nome_cliente, c.regiao, f.titulo, a.data_aluguel from clientes c
inner join alugueis a on
c.id_cliente = a.id_cliente
inner join filmes f on
f.id_filme = a.id_filme
...
select c.nome_cliente, count(a.data_aluguel) as qtd_alugueis from clientes c
 inner join alugueis a on
c.id_cliente = a.id_cliente
group by c.nome_cliente order by qtd_alugueis desc;
...
select f.titulo, count(a.data_aluguel) as qtd_alugueis from filmes f
inner join alugueis a on
f.id_filme = a.id_filme
group by f.titulo order by qtd_alugueis desc;
...
select f.titulo, round(avg(a.nota), 2) as media_notas from filmes f
inner join alugueis a on
f.id_filme = a.id_filme
where nota is not null
group by f.titulo;
...
select f.titulo, count(a.id_ator) as qtd_atores from filmes f
inner join atuacoes a on
f.id_filme = a.id_filme
group by f.titulo;
...
select c.regiao, count(a.data_aluguel) as qtd_alugueis from clientes c
inner join alugueis a on
c.id_cliente = a.id_cliente
group by c.regiao order by qtd_alugueis desc;
...
select f.titulo, count(a.data_aluguel) as qtd_alugueis, round(sum(f.preco_aluguel), 2) as preco_total from filmes f
inner join alugueis a on
f.id_filme = a.id_filme
group by f.titulo order by preco_total desc;
